...
export class UsuarioGitHubValidator {
  static validate(response: unknown): UsuarioGitHub {
    ...
    if (!('login' in response)) {
      throw new Error('Key/property Error.');
    }
    if (!('id' in response)) {
      throw new Error('Key/property Error.');
    }
    ...
    if (!this.isString(response.login)) {
      throw new Error('Type Error.');
    }
    if (!this.isNumber(response.id)) {
      throw new Error('Type Error.');
    }
    if (!this.isString(response.html_url)) {
      throw new Error('Type Error.');
    }
    if (!this.isNumber(response.public_repos)) {
      throw new Error('Type Error.');
    }
