...
export async function lerArquivo(): Promise<UsuarioGitHub[]> {
  try {
    ...
    return JSON.parse(usuariosString) as UsuarioGitHub[];
  } catch {
...
function verificarUsuarioExistente(usuarios: UsuarioGitHub[], user: UsuarioGitHub): boolean {
  ...
  usuarios.forEach((usuario) => {
    if (usuario.getLogin === user.getLogin) {
      usuarioExiste = true;
    }
  });
...
export async function salvarUsuario(user: UsuarioGitHub): Promise<boolean> {
  ...
  if (!usuarios) {
    //se não tiver nada dentro do arquivo
  ...
  const usuarioExistente = verificarUsuarioExistente(usuarios, user);
